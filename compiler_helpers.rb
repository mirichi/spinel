...
class Compiler
 ...
 # Issue #841: RationalNode literal slots.
  ...
  def grow_nodes(new_size)
    if new_size > @nd_count
      diff = new_size - @nd_count
      @nd_type.concat(Array.new(diff, ""))
      @nd_name.concat(Array.new(diff, ""))
      @nd_value.concat(Array.new(diff, 0))
      @nd_line.concat(Array.new(diff, 0))
      @nd_file.concat(Array.new(diff, 0))
      @nd_col.concat(Array.new(diff, 0))
      @nd_content.concat(Array.new(diff, ""))
      @nd_flags.concat(Array.new(diff, 0))
      @nd_operator.concat(Array.new(diff, ""))
      @nd_binop.concat(Array.new(diff, ""))
      @nd_callop.concat(Array.new(diff, ""))
      @nd_unescaped.concat(Array.new(diff, ""))
      @nd_receiver.concat(Array.new(diff, -1))
      @nd_arguments.concat(Array.new(diff, -1))
      @nd_body.concat(Array.new(diff, -1))
      @nd_block.concat(Array.new(diff, -1))
      @nd_parameters.concat(Array.new(diff, -1))
      @nd_predicate.concat(Array.new(diff, -1))
      @nd_subsequent.concat(Array.new(diff, -1))
      @nd_else_clause.concat(Array.new(diff, -1))
      @nd_left.concat(Array.new(diff, -1))
      @nd_right.concat(Array.new(diff, -1))
      @nd_constant_path.concat(Array.new(diff, -1))
      @nd_superclass.concat(Array.new(diff, -1))
      @nd_rest.concat(Array.new(diff, -1))
      @nd_keyword_rest.concat(Array.new(diff, -1))
      @nd_rescue_clause.concat(Array.new(diff, -1))
      @nd_ensure_clause.concat(Array.new(diff, -1))
      @nd_expression.concat(Array.new(diff, -1))
      @nd_target.concat(Array.new(diff, -1))
      @nd_pattern.concat(Array.new(diff, -1))
      @nd_key.concat(Array.new(diff, -1))
      @nd_reference.concat(Array.new(diff, -1))
      @nd_collection.concat(Array.new(diff, -1))
      @nd_stmts.concat(Array.new(diff, ""))
      @nd_args.concat(Array.new(diff, ""))
      @nd_requireds.concat(Array.new(diff, ""))
      @nd_optionals.concat(Array.new(diff, ""))
      @nd_keywords.concat(Array.new(diff, ""))
      @nd_elements.concat(Array.new(diff, ""))
      @nd_parts.concat(Array.new(diff, ""))
      @nd_conditions.concat(Array.new(diff, ""))
      @nd_exceptions.concat(Array.new(diff, ""))
      @nd_targets.concat(Array.new(diff, ""))
      @nd_rights.concat(Array.new(diff, ""))
      @nd_posts.concat(Array.new(diff, ""))
      @nd_new_name.concat(Array.new(diff, -1))
      @nd_old_name.concat(Array.new(diff, -1))
      @nd_names.concat(Array.new(diff, ""))
      @nd_inferred_type.concat(Array.new(diff, ""))
      @nd_scope_names.concat(Array.new(diff, ""))
      @nd_scope_types.concat(Array.new(diff, ""))
      @nd_rat_num.concat(Array.new(diff, ""))
      @nd_rat_den.concat(Array.new(diff, ""))
      @nd_count = new_size
    end
  end
